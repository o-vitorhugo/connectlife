...
roteador.post("/cadastro", async (req, res) => {
  try {
    const { nome, email, senha, papel, idade, telefone, endereco, habilidades, interesses } = req.body
...
roteador.put("/perfil", verificarToken, async (req, res) => {
  try {
    const { nome, telefone, endereco, habilidades, interesses, idade } = req.body
...
roteador.get("/informacoes", verificarToken, verificarPapel(["VOLUNTARIO", "IDOSO"]), async (req, res) => {
  ...
  try {
    const usuarios = await Usuario.findAll({
      attributes: ["id", "nome", "papel", "idade", "telefone", "habilidades", "interesses"],
      order: [["nome", "ASC"]],
    })
